...
%  Application object of VFS (Video FluoroScopy) analyzer.
  ...
  methods (Access = public)
    ...
    %  object.
    %
    function self = vfspApp (debug)
      self.debug = debug;
      if self.debug
        % Create an empty log file.
        FID = fopen(self.logFile, 'w');
        fclose (FID);

        self.log ('****');
        self.log (string(datetime('now','TimeZone','local','Format','d-MMM-y HH:mm:ss Z')));
        self.log ('****');
      end
      ...
      for i = 1:size(self.buttonTitles, 2)
        ...
        currentButtonDimension = currentButtonDimension + [0 -buttonHeight 0 0];
      end
    ...
    function savePreferences (self)
      ...
      coinMarkerSelectionCount = self.pref_coinMarkerSelectionCount;
    ...
    %  log
    %
    %  This function prints message to a log file.
    %
  ...
  methods (Access = private)
    ...
    function loadPreferences (self)
      ...
      if exist (preferencesFile, 'file') == 2
        ...
        self.pref_coinMarkerSelectionCount = coinMarkerSelectionCount;
    ...
    function showAboutDialog (self, src, event)
     % Create a dialog
      ...
      uicontrol (dialogWindow, ...
        'Style', 'text', ...
        'String', 'Copyright (c) 2015-2018. All rights reserved.', ...
        'HorizontalAlignment', 'center', ...
    ...
    function quitApplication (self, src, event)
      ...
      for i = 1:countDocuments 
        answer = documents(i).closeDocument (self);
